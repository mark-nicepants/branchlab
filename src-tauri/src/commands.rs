...
use crate::planning::SuggestedPlan;
...
pub async fn list_project_prs(
    ...
    let (account, owner, repo) = github.resolve_account_off_thread(&root, override_id.as_deref()).await?;
...
pub async fn task_suggest_plan(
    ...
    crate::planning::suggest_plan(&parent_id, &app, &registry, &chat, &tasks).await
}
...
pub async fn task_intake(
    ...
    crate::planning::intake(&parent_id, &content, &app, &registry, &chat, &tasks).await
}
