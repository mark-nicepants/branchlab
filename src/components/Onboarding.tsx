    ...
    <div className="flex h-screen items-center justify-center bg-background text-foreground">
      <div className="w-full max-w-lg rounded-xl border border-border bg-card p-8">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          BranchLab drives the{" "}
          <code className="rounded bg-muted px-1">opencode</code> CLI and uses{" "}
          <code className="rounded bg-muted px-1">git</code> for worktrees.
          Let's make sure both are installed.
        </p>
        ...
        <ul className="my-6 flex flex-col gap-2.5">
          ...
          <ToolRow
            name="git"
            status={env.git}
            hint="xcode-select --install   (macOS)"
          />
        </ul>
      ...
      <div className="min-w-0">
        ...
        {status.found ? (
          <div className="truncate text-xs text-muted-foreground">
            ...
            <code className="rounded bg-muted px-1">{status.path}</code>
          </div>
        ) : (
          <div className="text-xs text-muted-foreground">
            Not found on PATH. Install with:{" "}
            <code className="rounded bg-muted px-1">{hint}</code>
          </div>
        )}
      </div>
    </li>
